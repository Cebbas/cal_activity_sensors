"""Tests for fetch_matching_events: querying sources, filtering, merging."""
from unittest.mock import AsyncMock, MagicMock

from homeassistant.util import dt as dt_util

from custom_components.cal_activity.activity import fetch_matching_events
...
async def test_merges_events_from_multiple_sources_sorted_by_start():
    ...
    events, failed = await fetch_matching_events(hass, ["calendar.a", "calendar.b"], None, START, END)
    assert failed == []
...
async def test_missing_uid_falls_back_to_summary():
    hass = _hass_returning(
        {"calendar.a": [{"summary": "Ingen uid", "start": "2026-03-20T09:00:00+00:00", "end": "2026-03-20T10:00:00+00:00"}]}
    )
    events, _failed = await fetch_matching_events(hass, ["calendar.a"], None, START, END)
    assert events[0].uid == "Ingen uid"


async def test_all_day_event_parsed_as_date_not_datetime():
    hass = _hass_returning({"calendar.a": [{"uid": "1", "summary": "Lovdag", "start": "2026-06-01", "end": "2026-06-02"}]})
    events, _failed = await fetch_matching_events(hass, ["calendar.a"], None, START, END)
...
async def test_filter_rule_excludes_non_matching_events():
    hass = _hass_returning(
        {
            "calendar.a": [
    ...
    rule = {"include": ["fotboll"]}
    events, _failed = await fetch_matching_events(hass, ["calendar.a"], rule, START, END)
